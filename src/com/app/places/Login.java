	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		btnLogin.setOnClickListener(new OnClickListener() {
			public void onClick(View view) {
				Log.d(TAG, "Login");
				_dialog = ProgressDialog.show(Login.this, "",
	...
	public class LoginValidator extends AsyncTask<Integer, String, Integer> {
		...
		protected Integer doInBackground(Integer... params) {
			...
			try {
				db.open();
				ResultSet rs = db
						.executeQuery("SELECT profile.id, profile.login, profile.password "
								+ " FROM profile"
								+ " WHERE profile.login = "
								+ user
								+ "  AND "
		...
		protected void onPostExecute(Integer result) {
			...
			if (result > 0) {
				...
				// passa o id para a próxima atividade
				...
				_dialog.setButton(DialogInterface.BUTTON_NEGATIVE, "Ok",
				...
				_dialog.show();
				// _dialog = ProgressDialog.show(Login.this, "Error",
				// "Login does not match", false, true);
			}
